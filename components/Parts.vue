...
<div>
    <div v-show="parts.length !== 0">
        <v-card v-if="titleParts" outlined class="pa-2 pl-4 mb-2" style="border-left: 4px solid black!important;">
            <h1 style="font-size: 23px">{{ titleParts }}</h1>
        </v-card>
        <v-card class="bampart-table"
                tile
                id="scroll-index">
            <table class="table">
                <thead>
            ...
            <div class="bampart-pagination">
                <div class="bampart-pag text-xs-center" style="width: 100%;">
                    <v-pagination
                            v-model="page"
                            :length="totalPages"
                            :total-visible="7"
    ...
    <div v-if="parts.length === 0">
        <v-alert :value="true"
                 color="#424242"
                 dark
        >Запчасть не найдена. Измените параметры поиска</v-alert>
    </div>
</div>
</template>
<script>
    export default {
        ...
        computed: {
            ...
            totalPages() {
                return this.$store.getters['parts/totalPages']
            },
            totalParts() {
                return this.$store.getters['parts/totalParts']
            },
            article() {
                return this.$store.getters['parts/article']
            },
            titleParts() {
            ...
            page: {
                get () {
                    return this.$store.state.parts.page
                },
                set (value) {
                    this.$store.commit('parts/setPage', value)
                }
            }
        },
